...
def print_table(table):
    ...
    print(*[str(i).center(max_width) for i in table.get('header',[])], sep='|')
    for row in table.get('rows',[]):
        print(*[str(i).center(max_width) for i in row], sep='|')

def calc_max_temp_average(table):
    max_temps = []
    for row in in_memory_table.get('rows', []):
...
with open( ABSOLUTE_TSV_PATH, 'r' ) as file_sock:
    for line in file_sock:
        ...
        if len(row) > 0 and row[0].startswith('S'):
            in_memory_table['header'] = row
        else:
            in_memory_table['rows'].append(row)
